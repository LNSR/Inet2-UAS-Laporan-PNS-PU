...
function insertPengajuan($data) {
    ...
    $nb = htmlspecialchars($data["nama_proyek"]);
    $ket = mysqli_real_escape_string($conn, $data["ket"]);
    $status = "Sedang diajukan";
    $ket_petugas = "-";
    $tgl_lapor = date("Y-m-d");

    mysqli_query($conn, "INSERT INTO pengajuan VALUES('$id', '$np', '$jp', '$dp', '$nb', '$ket', '$status', '$ket_petugas', '$tgl_lapor')");
...
function registrasi($data) {
    ...
    if (mysqli_fetch_assoc($cek)) {
        echo "<script>alert('Username $username or NIP $nip was sudah terdaftar!');</script>";
        return false;
    }
...
function updatepengajuan($data) {
    global $conn;
    
    $id = $data['id'];
    $status = $data['status'];
    $ket_petugas = $data['ket_petugas'];
    mysqli_query($conn, "UPDATE pengajuan SET status = '$status', ket_petugas='$ket_petugas' WHERE id='$id'"); 
...
function searchpengajuan($keyword) {
    global $conn;
    $data = mysqli_query($conn, "SELECT * FROM pengajuan WHERE id = '$keyword'");
    return mysqli_affected_rows($conn);
}
